...
rocket_timer = pygame.time.get_ticks()  # Initialize the timer
ROCKET_COOLDOWN = 500  # Time in milliseconds between rockets
ROCKET_STEP = 0.3
...
while game_is_running:
    ...
    # Automatic rocket firing
    current_time = pygame.time.get_ticks()
    if current_time - rocket_timer > ROCKET_COOLDOWN:
        rocket_was_fired = True
        rocket_x = fighter_x + fighter_width / 2 - rocket_width / 2
        rocket_y = fighter_y - rocket_height
        rocket_timer = current_time  # Reset the timer
    ...
    alien_y += alien_speed
    if alien_y < rocket_y < alien_y + alien_height and alien_x < rocket_x < alien_x + alien_width:
        rocket_was_fired = False
        alien_x, alien_y = randint(0, screen_width - alien_width), alien_height
